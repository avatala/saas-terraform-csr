...
WITH
  ...
  FROM
    `PREFIX_NAME-admin-host.PREFIX_NAME_us_bigquery_host_prjct.gcp_billing_export_v1_BILLING_ACCOUNT_ID`
  WHERE
    usage_start_time >= TIMESTAMP("2022-11-01")
  --  AND project.id = "PREFIX_NAME-admin-host"
  GROUP BY
    project.id,
    sku.id,
    usage.unit ),
  ...
  SELECT
    ...
    billing_table.rate
  FROM
    `PREFIX_NAME-admin-host.PREFIX_NAME_us_bigquery_host_prjct.gke_cluster_resource_usage` AS resource_usage
  INNER JOIN
    billing_table
  ON
  ...
  SELECT
    ...
    FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', resource_usage.end_time) AS usage_end_time_ts,
    resource_usage.labels,
    resource_usage.namespace,
    resource_usage.resource_name,
    CASE
    ...
    FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', resource_usage.start_time) AS usage_start_time_ts,
    CASE
  ...
  consumption_based_cost_allocation AS (
      WITH
        ...
        SELECT
          ...
          billing_table.rate
        FROM
          `PREFIX_NAME-admin-host.PREFIX_NAME_us_bigquery_host_prjct.gke_cluster_resource_consumption` AS resource_usage
        INNER JOIN
          billing_table
        ON
          resource_usage.sku_id = billing_table.sku_id
        ...
        SELECT
          ...
          FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', filtered_resource_usage.end_time) AS usage_end_time_ts,
          filtered_resource_usage.labels,
          filtered_resource_usage.namespace,
          filtered_resource_usage.resource_name,
          CASE
          ...
          FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', filtered_resource_usage.start_time) AS usage_start_time_ts,
          CASE
            WHEN resource_name = 'cpu' THEN amount/3600
            WHEN resource_name = 'memory' THEN amount/(3600*POW(2,30))
            END
