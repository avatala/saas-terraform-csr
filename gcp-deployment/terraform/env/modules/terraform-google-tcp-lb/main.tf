...
resource "google_compute_global_forwarding_rule" "freemium_fr" {
  ...
  target                = google_compute_target_tcp_proxy.freemium.id
...
resource "google_compute_global_forwarding_rule" "enterprise_fw" {
  ...
  load_balancing_scheme = "EXTERNAL"
  port_range            = var.application_port
  target                = google_compute_target_tcp_proxy.enterprise.id
  ip_address            = google_compute_global_address.tcp_enterprise_ip.id
}

resource "google_compute_target_tcp_proxy" "freemium" {
  project         = var.project
  provider        = google-beta
  name            = "${var.name}-freemium-proxy"
  backend_service = google_compute_backend_service.freemium_backend.id
}

resource "google_compute_target_tcp_proxy" "enterprise" {
  project         = var.project
  provider        = google-beta
  name            = "${var.name}-enterprise-proxy"
  backend_service = google_compute_backend_service.freemium_backend.id
}
