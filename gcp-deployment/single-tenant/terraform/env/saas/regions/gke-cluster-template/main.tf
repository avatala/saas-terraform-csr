...
module "main_vpc_gke_private_clusters" {
  ...
  service_account                  = "${_CUSTOMER_NAME}-saas-gke-sa@${data.terraform_remote_state.service_project_id.outputs.service_projects.service_project_01["project_id"]}.iam.gserviceaccount.com"
  resource_usage_export_dataset_id = "${_CUSTOMER_NAME}_saas_dsid"
  cluster_resource_labels = merge(
    each.value.cluster_resource_labels,
    tomap(
      {
        "mesh_id" = "proj-${data.terraform_remote_state.service_project_id.outputs.service_projects.service_project_01["project_number"]}"
      }
    )
  )
